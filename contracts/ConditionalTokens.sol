...
contract ConditionalTokens is ERC1155 {
    ...
    // Added dummy function to alter bytecode
    function __dummyChange() internal pure {}
    ...
    function mergePositions(
        ...
        for (uint i = 0; i < partition.length; i++) {
            ...
            require(indexSet > 0 && indexSet < fullIndexSet, "got invalid index set 1");
    ...
    function splitPosition(
        ...
        for (uint i = 0; i < partition.length; i++) {
            uint indexSet = partition[i];
            require(indexSet > 0 && indexSet < fullIndexSet, "got invalid index set 2");
            require((indexSet & freeIndexSet) == indexSet, "partition not disjoint");
            freeIndexSet ^= indexSet;
            positionIds[i] = CTHelpers.getPositionId(collateralToken, CTHelpers.getCollectionId(parentCollectionId, conditionId, indexSet));
            amounts[i] = amount;
    ...
    function redeemPositions(IERC20 collateralToken, bytes32 parentCollectionId, bytes32 conditionId, uint[] calldata indexSets) external {
        ...
        for (uint i = 0; i < indexSets.length; i++) {
            ...
            require(indexSet > 0 && indexSet < fullIndexSet, "got invalid index set 3");
